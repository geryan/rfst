...
rfa_boundary <- spTransform(rfa_boundary, ch_rst@crs)
```
...
```{r}
#plot(gg_hab_suit[[50]], box = FALSE, axes = FALSE, col = viridis(100))
plot(gg_hab_suit[[50]])
plot(rfa_boundary, add = TRUE)
```
...
```{r}
gg_results_10 <- gg_results
saveRDS(object = gg_results_10, file = "output/gg_results_10.Rds")
```
...
```{r}
plot(gg_results[1], type = "raster", stages = 1, timesteps = c(1:5))
```
