...
```{r pg_gg_pa_ch}
pg_gg_pa_ch <- pg.pa(gg_pa_ch %>% mutate(pseudo_abs = "vba") %>% rbind(bg_gg %>% mutate(pseudo_abs = "pseudo_absences")), ch_rfa) +
...
```{r pg_lb_pa_ch}
pg_lb_pa_ch <- pg.pa(lb_pa_ch %>% mutate(pseudo_abs = "vba") %>% rbind(bg_lb %>% mutate(pseudo_abs = "pseudo_absences")), ch_rfa) +
  facet_grid(.~pseudo_abs)
...
##### Mid fecundity
```{r gg_trans_mat_fm}
tm_gg_fm <- matrix(c(0.00,0.00,0.50,
                     0.50,0.00,0.00,
                     0.00,0.85,0.85),
                   nrow = 3,
                   ncol = 3,
...
```{r l_gg_4_1}
l_gg_4_1 <- landscape(
  population = p0_gg_4_1,
  suitability = s4_p_gg_1,
  carrying_capacity = cc_gg_4_1
)
```

###### Model 1 ps
```{r l_gg_4_1_ps}
l_gg_4_1_ps <- landscape(
  population = p0_gg_4_1_ps,
  suitability = s4_p_gg_1_ps,
  carrying_capacity = cc_gg_4_1_ps
)
```
