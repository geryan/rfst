...
*change mask to ch layer it's silly using this one*

ch_rst <- raster(<path to existing landis eco rst>)
mask <- ch_rst

extent(mask) <- round(extent(ch_rst))
...
#### Initial variables
```{r ari_iv}
ari_iv <- stack(lvdaw, lvdma, lvdmi, lvdsw, ahr, tho)

names(ari_iv) <- c("lvdaw", "lvdma", "lvdmi", "lvdsw", "ahr", "tho")
```
...
**NEED TO ENSURE PREDICTIONS FROM CLIMATE SCENARIOS MATCH WORLDCLIM SCALE (PROB NEED TO SCALE DOWN TEMPERATURES)**
