...
```{r s1_ip_gg}
s1_ip_gg <- predict(s1_iv_gg, s1_mod_gg, type = "response", n.trees = s1_mod_gg$gbm.call$best.trees)
...
### Scenario 3
```{r s3_mod_gg}
s3_mod_gg <- gbm.step(data = s3_md_gg, gbm.x = 2:8, gbm.y = 1, family = "bernoulli", tree.complexity = 5, learning.rate = 0.001, step.size = 1, bag.fraction = 0.5, prev.stratify = FALSE, verbose = FALSE, max.trees = 1000)
```
```{r sumary s3_mod_gg}
summary(s3_mod_gg)
```
...
```{r s3_ip_gg}
s3_ip_gg <- predict(s3_iv_gg, s3_mod_gg, type = "response", n.trees = s3_mod_gg$gbm.call$best.trees)
...
```{r s4_ip_gg}
s4_ip_gg <- predict(s4_iv_gg, s4_mod_gg, type = "response", n.trees = s4_mod_gg$gbm.call$best.trees)
...
### Scenario 1
```{r s1_mod_lb}
s1_mod_lb <- gbm.step(data = s1_md_lb, gbm.x = 2:8, gbm.y = 1, family = "bernoulli", tree.complexity = 5, learning.rate = 0.001, step.size = 1, bag.fraction = 0.5, prev.stratify = FALSE, verbose = FALSE, max.trees = 1000)
```
```{r sumary s1_mod_lb}
summary(s1_mod_lb)
```
...
```{r s1_ip_lb}
s1_ip_lb <- predict(s1_iv_lb, s1_mod_lb, type = "response", n.trees = s1_mod_lb$gbm.call$best.trees)
...
### Greater Glider
```{r}
s1_hs_gg <- c(list(s1_ip_gg), s1_fp_gg) %>% stack


s1_hs_gg <- s1_hs_gg[[1:50]]
...
```{r}
gg_popN <- stack(replicate(ncol(gg_trans_mat), s1_hab_k_gg))
#gg_popN <- stack(replicate(ncol(gg_trans_mat), floor(gg_hab_suit*200)))

gg_popN <- gg_popN*gg_stable_states

idx <- which(!is.na(s1_hs_gg[[1]][]) & s1_hs_gg[[1]][] < 0.95)
# 
...
gg_results_1 <- simulation(landscape = gg_landscape,
...
```{r}
plan(sequential, workers = 1)
gg_results_1_20 <- simulation(landscape = gg_landscape,
                         population_dynamics = gg_pop_dynamics,
                         habitat_dynamics = NULL,
                         timesteps = 20,
                         replicates = 1)
...
gg_results_20_20 <- simulation(landscape = gg_landscape,
                         population_dynamics = gg_pop_dynamics,
                         habitat_dynamics = NULL,
                         timesteps = 20,
                         replicates = 20)
...
```{r}
saveRDS(object = gg_results_1_20, file = "output/pva_results/gg_results_1_20.Rds")
```


```{r}
plot(gg_results)
plot(gg_results[1], type = "raster", stages = 0, timesteps = c(1,5))
plot(gg_results[1], type = "raster", stages = 0, animate = TRUE, timesteps = 1:5)
```
