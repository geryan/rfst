...
fire_history <- read_sf("data/shapefiles/vicfires/fire_history_lastburnt.shp") %>%
  ...
  mask(mask = ch_mask, filename = "output/landscape_vars/fire_history.grd")
...
logging_history <- read_sf("data/shapefiles/viclogging/lastlog25.shp") %>%
  ...
  mask(mask = ch_mask, filename = "output/landscape_vars/logging_history.grd")
...
#### Scenario 1:
```{r s1_tsf}
s1_tsf <- vector("list", ntimesteps + 1)
s1_fihi <- vector("list", ntimesteps + 1)
s1_firesev <- lapply(s1_vars_landis, FUN = function(x){x[["firesev"]]})

s1_fihi[[1]] <- rst.op(input1 = s1_firesev[[1]],
                   input2 = fire_history,
                   op = "history",
                   proj_mask = ch_mask,
                   filename = sprintf("%s/%s_%s_%03d.grd",
                                      "output/landscape_vars/fire_history",
                                      "s1",
                                      "fihi",
                                      year0),
                   layernames = "fihi",
...
for (i in 2:(ntimesteps+1)){
  s1_fihi[[i]] <- rst.op(input1 = s1_firesev[[i]],
                   input2 = s1_fihi[[i-1]],
                   op = "history",
                   proj_mask = ch_mask,
                   filename = sprintf("%s/%s_%s_%03d.grd",
...
for(i in 1:(ntimesteps + 1)){
  ...
  s1_tsf[[i]] <- rst.op(input1 = s1_firesev[[i]],
                        input2 = s1_fihi[[i]],
                        op = "timesince",
                        proj_mask = ch_mask,
                        filename = sprintf("%s/%s_%s_%03d.grd",
                                            "output/landscape_vars/tsf",
                                            "s1",
                                            "tsf",
                                            year0 + i - 1),
                         layernames = "tsf",
...
s1_fire_vars <- get.disturbance(disturbances =  lapply(s1_vars_landis, FUN = function(x){x[["firesev"]]}),
                                disturnce_history = fire_history,
...
```{r s1_iv_gg}
s1_iv_gg <- stack(s1_vars_landis[[c(gglv)]], clim_iv)
