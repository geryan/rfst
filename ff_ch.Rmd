...
## Distribution model variable selection
...
```{r check again gg_cor}
which(cor(md_gg) > 0.7 && cor(md_gg) != 1) 
```
#### Remaining scenarion variables
...
#####  Recompile LB LANDIS vars
```{r lblv recompile}
lblv <- c("lbm", "prop_oge_3h")
```

##### Scenario 1Variable layers
```{r s1_v_lb recompile}
s1_v_lb <- s1_vars_landis %>%
  sapply("[[", lblv) %>%
  mapply(clim_vars_4.5,
...
Check agian no  highly correlated variables
```{r check again lb_cor}
which(cor(md_lb) > 0.7 && cor(md_lb) != 1) 
```
...
### Greater Glider
```{r}
aa <- t(data.frame(names(md_gg), 1:length(names(md_gg))))
...
```{r brt_gg}
brt_gg <- gbm.step(data = md_gg, gbm.x = c(2:6), gbm.y = 1, family = "bernoulli", tree.complexity = 4, learning.rate = 0.001, step.size = 1, bag.fraction = 0.5, prev.stratify = FALSE, verbose = TRUE, max.trees = 5000)
```
...
```{r initial pred brt_gg}
ip_brt_gg <- predict(s1_v_gg[[1]][[c(1:5)]], brt_gg, type = "response", n.trees = brt_gg$gbm.call$best.trees, filename = "output/habitat_pred/s1_p_brt_gg-000.tif", overwrite = TRUE)
