...
```{r}
s1_mod_gg <- gbm.step(data = s1_md_gg, gbm.x = 3:8, gbm.y = 1, family = "bernoulli", tree.complexity = 5, learning.rate = 0.001, step.size = 1, bag.fraction = 0.5, prev.stratify = FALSE, verbose = FALSE, max.trees = 1000)
```
```{r}
summary(s1_mod_gg)
```
...
```{r}
s1_ip_gg <- predict(s1_iv_gg[[2:7]], s1_mod_gg, type = "response", n.trees = s1_mod_gg$gbm.call$best.trees)
...
```{r}
saveRDS(object = gg_results, file = "output/pva_results/gg_results.Rds")
```
...
### Greater Glider
```{r}
s1_hs_lb <- c(list(s1_ip_lb), s1_fp_lb) %>% stack
...
```{r}
lb_popN <- stack(replicate(ncol(lb_trans_mat), s1_hab_k_lb))
#lb_popN <- stack(replicate(ncol(lb_trans_mat), floor(lb_hab_suit*200)))

lb_popN <- lb_popN*lb_stable_states

idx <- which(!is.na(s1_hs_lb[[1]][]) & s1_hs_lb[[1]][] < 0.95)
# 
# lb_p <- foreach(i = 1:nlayers(lb_popN)) %do% {
#   m <- ceiling(cellStats(lb_popN[[i]], max, na.rm = TRUE))
#     pop <- calc(lb_popN[[i]], fun = function(x){if(is.na(x)) x else rbinom(prob = (x/m), size = m, n = 1)})
#     pop[idx] <- 0
#     pop
# } ## Not working right investigate why no integeers
# 
...
lb_pop <- lb_popN
lb_pop[!is.na(lb_pop)] <- 0
lb_pop[[1]][sample(idx, 10000)] <- 1
lb_pop[[2]][sample(idx, 10000)] <- 1
lb_pop[[3]][sample(idx, 10000)] <- 1

lb_pop <- lb_pop*ch_mask

#registerDoMC(cores = 4)
#lb_pop <- stack(
#  foreach(i = 1:nlayers(lb_popN)) %dopar% {
#    m <- ceiling(cellStats(lb_popN[[i]], max, na.rm = TRUE))
#    pop <- calc(lb_popN[[i]], fun = function(x) if(is.na(x)) x else rbinom(prob = (x/m), size = m, n = 1))
#    pop[idx] <- 0
#    pop
#  })
names(lb_pop) <- colnames(lb_trans_mat)
#plot(lb_pop, box = FALSE, axes = FALSE, col = viridis(100))

TotpopN <- sum(cellStats(lb_pop, 'sum', na.rm = TRUE)) # Get total population size to check sensible
init_pop_size <- sum(lb_pop)
#plot(init_pop_size, box = FALSE, axes = FALSE, col = viridis(25))
...
```{r}
lb_landscape <- landscape(population = lb_pop,
                          suitability = s1_hs_lb,
                          carrying_capacity = s1_hab_k_lb)

lb_pop_dynamics <- population_dynamics(change = growth(transition_matrix = lb_trans_mat,
                                                       global_stochasticity = 0.1),
                                       dispersal = fast_dispersal(
                                         dispersal_kernel = exponential_dispersal_kernel(
                                           distance_decay = 8000)),
