...
```{r}
l1 <- landscape(
  population = simset_lb$init_pop[[1]],
  suitability = simset_lb$hs[[1]],
  carrying_capacity = simset_lb$cc[[1]]
)
```
...
```{r}
zz <- simset_lb %>%
  dplyr::select(scenario, rep, pva_pops) %>%
  rowwise %>%
  mutate(
    emp = emp(pva_pops),
  ...
  mutate(scenario = case_when(scenario == 1 ~ "BAU",
...
```{r}
aa <- zz %>%
  dplyr::select(scenario, rep, all.emp) %>%
  unnest(all.emp) %>%
...
```{r}
ggplot(zz, aes(x = scenario, y = emp)) +
  geom_boxplot() +
  geom_point()
...
```{r}
ggplot(aa, aes(x = scenario, y = min.pop)) +
...
```{r}
psr(simset_lb28$pva_pops[[1]], stages = 0)
```


```{r}
zz <- simulation(landscape = aa$lsc[[1]], population_dynamics = aa$pd[[1]], timesteps = 50, replicates = 10)
```

```{r}
zz <- simulation(
  landscape = landscape(
    population = aa$init_pop
  ),
  population_dynamics = aa$pd,
  timesteps = 5,
  replicates = 1
)
```
