---
title: "Illustrative Results of STEPS Population Viability Analysis for Leadbeater's Possum and Greater Glider"
output:
  pdf_document:
    toc: TRUE
    toc_depth: 3
...
Results are illustrative of the types of output information available from the process, and should not be considered definitive results of any given scenario or outcome.

Here we illustrate three potential landscape scenarios:

- Business as usual (ongoing harvest),
- Ceasing harvest after 30 years, and 
- No timber harvesting.

Each of these scenarios is modelled under the current planned burning regime, and following the climate pathway RCP 4.5, using the Access 1.0 model. It is important to note that it is vital to consdier other climate pathways in this modelling as this can have large effects on the forecasts of suitable habitat.

For each of the three scenarios we have run 10 LANDIS simulations, which each will result in different simulated landscapes.

Here we generally illustrate results for a single LANDIS simulation in each scenario.

For each of these 30 LANDIS simulations we complete a population viability analysis for each species. Each of these PVAs consists of 100 simulated populations.
...
Biomass of midstorey species and acacias
...
#### And additional run of this scenario
```{r fire_1_10 bau}
lp(adv_1_10$fire[[1]], twocol = TRUE)
```

```{r}
lp(adv_1_10$fire, twocol = TRUE)
```
...
```
### Fires in time 50 in each LANDIS run
Fires in all 10 runs of each of the three scenarios - illustratiing variation in fire occurrence in a single simulated timestep.
```{r}
...
```{r}
lp(firestack[[7:12]], twocol = TRUE)
```

```{r}
lp(firestack[[13:18]], twocol = TRUE)
```

```{r}
lp(firestack[[19:24]], twocol = TRUE)
```

```{r}
lp(firestack[[23:30]], twocol = TRUE)
```
