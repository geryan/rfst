...
load(file = "output/RData/11.1_pva_pevo_eg.RData")
```
...
```{r}
p_emp_pevo_ls <- ggplot(pvr %>% filter(climate_model == "ACCESS1-0")) +
  geom_boxplot(
    aes(
      y = scn,
      x = emp,
  ...
  scale_colour_manual(values = c("grey40", "grey10")) +
  labs(
...
```{r}
p_demp95_pevo_ls <- ggplot(pvr %>% filter(climate_model == "ACCESS1-0")) +
  geom_vline(xintercept = 0) +
  geom_boxplot(
    aes(
      y = scn,
...
```{r}
p_pex_pevo_cm <- ggplot(pvr %>% filter(landscape == "Central Highlands")) +
  geom_boxplot(
    aes(
      y = scn,
      x = prop_extant,
...
```{r}
p_tex_pevo_cm <- ggplot(pvr %>% filter(landscape == "Central Highlands")) +
  geom_boxplot(
    aes(
      y = scn,
      x = mean_tt_ex,
...
```{r}
p_dpex_pevo_cm <- ggplot(pvr %>% filter(landscape == "Central Highlands")) +
  geom_vline(xintercept = 0) +
  geom_boxplot(
    aes(
      y = scn,
